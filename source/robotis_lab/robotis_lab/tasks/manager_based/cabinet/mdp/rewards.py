...
def approach_ee_handle(env: ManagerBasedRLEnv, threshold: float) -> torch.Tensor:
    ...
    distance = torch.norm(handle_pos - end_effector_pos, dim=-1, p=2)
...
def align_ee_handle(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    and :math:`align_y` is the dot product of the x direction of the gripper and the -y direction of the handle.
    """
    end_effector_quat = env.scene["ee_frame"].data.target_quat_w[..., 0, :]
    handle_quat = env.scene["cabinet_frame"].data.target_quat_w[..., 0, :]

    end_effector_rot_mat = matrix_from_quat(end_effector_quat)
    ...
    # and the y direction of the gripper should be close to the -y direction of the handle
    # dot product of z and y should be large
    align_z = torch.bmm(end_effector_z.unsqueeze(1), -handle_x.unsqueeze(-1)).squeeze(-1).squeeze(-1)
    align_x = torch.bmm(end_effector_x.unsqueeze(1), handle_y.unsqueeze(-1)).squeeze(-1).squeeze(-1)
...
def approach_gripper_handle(env: ManagerBasedRLEnv, offset: float = 0.04) -> torch.Tensor:
    ...
    # If the reward is negative, set it to zero
    if torch.any(reward < 0):
        reward[reward < 0] = 0.0
