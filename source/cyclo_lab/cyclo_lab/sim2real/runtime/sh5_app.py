...
from cyclo_lab.sim2real.controllers.swerve_drive import SwerveModule
from cyclo_lab.sim2real.runtime.sh5_bridge import SH5ZenohRos2Bridge
from cyclo_lab.sim2real.specs import ffw_sh5 as sh5_cfg
from cyclo_lab.sim2real.specs.ffw_sh5 import FFW_SH5_ACTION_TOPICS
...
def _make_robot_cfg(usd_path: str) -> ArticulationCfg:
    ...
    robot_cfg.init_state.pos = sh5_cfg.ROBOT_POS
    return robot_cfg
...
def run_simulator(simulation_app, args_cli, sim: sim_utils.SimulationContext, scene: InteractiveScene, bridge: SH5ZenohRos2Bridge):
    ...
    step_period = 1.0 / sh5_cfg.STEP_HZ if sh5_cfg.STEP_HZ > 0 else 0.0
    publish_period = 1.0 / sh5_cfg.PUBLISH_HZ if sh5_cfg.PUBLISH_HZ > 0 else 0.0
    last_publish = 0.0
    last_step = time.time()
...
def main(args_cli, simulation_app):
    ...
    sim_cfg = sim_utils.SimulationCfg(
        ...
        dt=1.0 / sh5_cfg.STEP_HZ,
        render_interval=sh5_cfg.RENDER_INTERVAL,
    )
    sim = sim_utils.SimulationContext(sim_cfg)
    sim.set_camera_view(sh5_cfg.ISAAC_SIM_OVERVIEW_CAMERA_EYE, sh5_cfg.ISAAC_SIM_OVERVIEW_CAMERA_TARGET)
    ...
    bridge = SH5ZenohRos2Bridge(
        ...
        cmd_vel_timeout=sh5_cfg.CMD_VEL_TIMEOUT,
    )
